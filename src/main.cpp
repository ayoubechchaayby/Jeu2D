...
int main(int, char**) // Version special du main, ne pas modifier
{
  ...
  Ennemi ennemi3 (sprites_personnages, 2 * TAILLE_CASE, 4*TAILLE_CASE, 0, 4, HAUT);
  Ennemi ennemi4 (sprites_personnages, 3*TAILLE_CASE, 3 * TAILLE_CASE, 9, 0, DROITE);
  ...
  while (!quitter)
  {
    ...
    if (avatar.touche(ennemi1) || avatar.touche(ennemi2) || avatar.touche(ennemi3) || avatar.touche(ennemi4))
    {
      // Question 2.5
      avatar.perteVie();
    }
